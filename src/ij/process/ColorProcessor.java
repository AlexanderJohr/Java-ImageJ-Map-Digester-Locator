 ...
 * the ImageProcessor class from "KickAss Java Programming" by Tonny Espeset
 * (http://www.sn.no/~espeset).
 */
public class ColorProcessor extends ImageProcessor {
	...
	private static double rWeight = 1d / 3d, gWeight = 1d / 3d,
			bWeight = 1d / 3d;
	private double[] weights; // Overrides rWeight, etc. when set by
								// setWeights()
	...
	Image createBufferedImage() {
		...
		if (rgbRaster == null) {
			...
			rgbRaster = Raster.createWritableRaster(rgbSampleModel, dataBuffer,
					null);
		}
		if (image == null) {
	...
	/**
	 * Sets the default fill/draw value, where <code>value</code> is interpreted
	 * as an RGB int.
	 */
	...
	/**
	 * Sets the background fill value, where <code>value</code> is interpreted
	 * as an RGB int.
	 */
	...
	/**
	 * Uses a table look-up to map the pixels in this image from min-max to
	 * 0-255.
	 */
	...
	public void snapshot() {
		...
		if (snapshotPixels == null
				|| (snapshotPixels != null && snapshotPixels.length != pixels.length))
	...
	/**
	 * Returns a reference to the snapshot pixel array. Used by the
	 * ContrastAdjuster.
	 */
	...
	/**
	 * Fills pixels that are within roi and part of the mask. Does nothing if
	 * the mask is not the same as the the ROI.
	 */
	...
	/**
	 * Returns the 3 samples for the pixel at (x,y) in an array of int. Returns
	 * zeros if the the coordinates are not in bounds. iArray is an optional
	 * preallocated array.
	 */
	...
	/**
	 * Sets a pixel in the image using a 3 element (R, G and B) int array of
	 * samples.
	 */
	...
	/**
	 * Stores the specified real grayscale value at (x,y). Does nothing if (x,y)
	 * is outside the image boundary. The value is clamped to be in the range
	 * 0-255.
	 */
	...
	/**
	 * Converts the specified pixel to grayscale using the formula g=(r+g+b)/3
	 * and returns it as a float. Call setWeightingFactors() to specify
	...
	public float getPixelValue(int x, int y) {
		if (x >= 0 && x < width && y >= 0 && y < height) {
			...
			int b = c & 0xff;
			if (weights != null)
				return (float) (r * weights[0] + g * weights[1] + b
						* weights[2]);
			else
	...
	/**
	 * Returns an ImageStack with three 8-bit slices, representing hue,
	...
	/**
	 * Returns the specified plane (1=red, 2=green, 3=blue, 4=alpha) as a byte
	 * array.
	 */
	...
	/**
	 * Returns the specified plane (1=red, 2=green, 3=blue, 4=alpha) as a
	 * ByteProcessor.
	 */
	 ...
	 * 
	 * @param channel
	 *            Determines the color channel, 1=red, 2=green, 3=blue, 4=alpha
	 * @param bp
	...
	public void setRGB(byte[] R, byte[] G, byte[] B) {
		int c, r, g, b;
		for (int i = 0; i < width * height; i++)
			pixels[i] = 0xff000000 | ((R[i] & 0xff) << 16)
					| ((G[i] & 0xff) << 8) | B[i] & 0xff;
	}

	/**
	 * Sets the current pixels from 3 byte arrays (hue, saturation and
	 * brightness).
	 */
	...
	/**
	 * Performs the specified filter on the red, green and blue planes of this
	 * image.
	 */
	...
	public void scale(double xScale, double yScale) {
		...
		for (int y = ymin; y <= ymax; y++) {
			...
			for (int x = xmin; x <= xmax; x++) {
				...
				xsi = (int) xs;
				if (checkCoordinates
						&& ((xsi < xmin) || (xsi > xmax) || (ysi < ymin) || (ysi > ymax)))
	...
	/**
	 * Creates a new ColorProcessor containing a scaled copy of this image or
	 * selection.
	 * 
	 * @see ImageProcessor#setInterpolate
	 */
	...
	/**
	 * Uses averaging to creates a new ColorProcessor containing a downsized
	 * copy of this image or selection.
	 */
	public ImageProcessor makeThumbnail(int width2, int height2,
			double smoothFactor) {
	...
	public void convolve3x3(int[] kernel) {
		...
		int k1 = kernel[0], k2 = kernel[1], k3 = kernel[2], k4 = kernel[3], k5 = kernel[4], k6 = kernel[5], k7 = kernel[6], k8 = kernel[7], k9 = kernel[8];
		...
		for (int y = yMin; y <= yMax; y++) {
			...
			for (int x = xMin; x <= xMax; x++) {
				...
				gsum = k1 * ((p1 & 0xff00) >> 8) + k2 * ((p2 & 0xff00) >> 8)
						+ k3 * ((p3 & 0xff00) >> 8) + k4 * ((p4 & 0xff00) >> 8)
						+ k5 * ((p5 & 0xff00) >> 8) + k6 * ((p6 & 0xff00) >> 8)
						+ k7 * ((p7 & 0xff00) >> 8) + k8 * ((p8 & 0xff00) >> 8)
						+ k9 * ((p9 & 0xff00) >> 8);
				gsum /= scale;
				if (gsum > 255)
					gsum = 255;
				...
				bsum = k1 * (p1 & 0xff) + k2 * (p2 & 0xff) + k3 * (p3 & 0xff)
						+ k4 * (p4 & 0xff) + k5 * (p5 & 0xff) + k6
						* (p6 & 0xff) + k7 * (p7 & 0xff) + k8 * (p8 & 0xff)
						+ k9 * (p9 & 0xff);
				bsum /= scale;
				if (bsum > 255)
					bsum = 255;
				...
				pixels[offset++] = 0xff000000 | ((rsum << 16) & 0xff0000)
						| ((gsum << 8) & 0xff00) | (bsum & 0xff);
			}
			if (y % inc == 0)
				showProgress((double) (y - roiY) / roiHeight);
		}
	...
	public void filter(int type) {
		...
		for (int y = yMin; y <= yMax; y++) {
			...
			for (int x = xMin; x <= xMax; x++) {
				...
				p8 = p9;
				p9 = pixels2[offset + rowOffset + 1];
				rsum = (p1 & 0xff0000) + (p2 & 0xff0000) + (p3 & 0xff0000)
						+ (p4 & 0xff0000) + (p5 & 0xff0000) + (p6 & 0xff0000)
						+ (p7 & 0xff0000) + (p8 & 0xff0000) + (p9 & 0xff0000);
				gsum = (p1 & 0xff00) + (p2 & 0xff00) + (p3 & 0xff00)
						+ (p4 & 0xff00) + (p5 & 0xff00) + (p6 & 0xff00)
						+ (p7 & 0xff00) + (p8 & 0xff00) + (p9 & 0xff00);
				bsum = (p1 & 0xff) + (p2 & 0xff) + (p3 & 0xff) + (p4 & 0xff)
						+ (p5 & 0xff) + (p6 & 0xff) + (p7 & 0xff) + (p8 & 0xff)
						+ (p9 & 0xff);
				pixels[offset++] = 0xff000000 | ((rsum / 9) & 0xff0000)
						| ((gsum / 9) & 0xff00) | (bsum / 9);
			}
			if (y % inc == 0)
				showProgress((double) (y - roiY) / roiHeight);
		}
	...
	public void convolve(float[] kernel, int kernelWidth, int kernelHeight) {
		...
		setRGB((byte[]) r2.getPixels(), (byte[]) g2.getPixels(),
				(byte[]) b2.getPixels());
	}

	/**
	 * Sets the weighting factors used by getPixelValue(), getHistogram() and
	 * convertToByte() to do color conversions. The default values are 1/3, 1/3
	 * and 1/3. Check "Weighted RGB Conversions" in
	 * <i>Edit/Options/Conversions</i> to use 0.299, 0.587 and 0.114.
	 * 
	 * @see #getWeightingFactors
	 * @see #setRGBWeights
	 */
	public static void setWeightingFactors(double rFactor, double gFactor,
			double bFactor) {
		rWeight = rFactor;
		gWeight = gFactor;
		bWeight = bFactor;
	...
	/**
	 * Returns the values set by setRGBWeights(), or null if setRGBWeights() has
	 * not been called.
	 */
	...
	/**
	 * Returns a FloatProcessor with one color channel of the image. The roi and
	 * mask are also set for the FloatProcessor.
	 * 
	 * @param channelNumber
	 *            Determines the color channel, 0=red, 1=green, 2=blue
	 * @param fp
	 *            Here a FloatProcessor can be supplied, or null. The
	 *            FloatProcessor is overwritten by this method (re-using its
	 *            pixels array improves performance).
